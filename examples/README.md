...
**运行方式**:
```bash
python -m aiotest -f examples/simple_http_test.py
```
...
**运行方式**:
```bash
python -m aiotest -f examples/basic_load_shape.py
```
...
**运行方式**:
```bash
python -m aiotest -f examples/api_performance_test.py --prometheus-port 8089
```
...
**运行方式**:
```bash
python -m aiotest -f examples/concurrent_example.py
```
...
**运行方式**:
```bash
python -m aiotest -f examples/weight_and_wait.py
```
...
**运行方式**:
```bash
python -m aiotest -f examples/events_example.py
```
...
**运行方式**:
```bash
python -m aiotest -f examples/request_verification.py
```
...
```bash
# 启动 Master
python -m aiotest -f examples/distributed_example.py --master --expect-workers 2

# 启动 Worker（在另一台机器上）
python -m aiotest -f examples/distributed_example.py --worker
```
...
**运行方式**:
```bash
python -m aiotest -f examples/database_performance_test.py
```
...
**运行方式**:
```bash
python -m aiotest -f examples/microservices_test.py
```
...
**运行方式**:
```bash
python -m aiotest -f examples/seckill_scenario.py
```
...
**运行方式**:
```bash
python -m aiotest -f examples/pause_resume_test.py
```
...
- [database_performance_test.py](database_performance_test.py) - 数据库性能测试
...
# 指定日志级别
python -m aiotest -f examples/basic.py --loglevel DEBUG

# 指定日志文件
python -m aiotest -f examples/basic.py --logfile test.log
```
...
# 启动 Master
python -m aiotest -f examples/distributed_example.py \
...
```bash
# 启动 Prometheus 指标
python -m aiotest -f examples/basic.py --prometheus-port 8089
