...
- [核心功能](#核心功能)
- [连接池配置](#连接池配置)
- [核心类：HTTPClient](#核心类-httpclient)
- [响应上下文管理器：ResponseContextManager](#响应上下文管理器-responsecontextmanager)
- [调用逻辑流程](#调用逻辑流程)
- [流程图](#流程图)
- [配置参数](#配置参数)
- [使用示例](#使用示例)
...
## 响应上下文管理器：ResponseContextManager
...
2. **进入上下文** → 调用 `__aenter__()` 初始化连接池和会话
3. **发送请求** → 调用 `request()` 方法发送 HTTP 请求
4. **执行请求** → 内部调用 `_request_with_retry()` 执行带重试的请求
5. **处理响应** → 使用 `ResponseContextManager` 处理响应
6. **记录指标** → 调用 `_record_request_metrics()` 记录性能指标
7. **退出上下文** → 调用 `__aexit__()` 关闭会话和连接池

### 请求执行流程

1. **准备请求** → 构建请求参数，添加默认头
2. **记录开始** → 记录请求开始时间和日志
3. **执行请求** → 调用 `_request_with_retry()` 执行请求
4. **处理响应** → 读取响应数据，解析为 JSON 或文本
5. **计算指标** → 计算响应时间、响应大小等指标
6. **记录完成** → 记录请求完成日志和指标
7. **处理错误** → 如果发生错误，记录错误信息和指标

### 重试机制流程

1. **检查重试设置** → 如果 `max_retries` 为 0，直接执行请求
2. **设置重试条件** → 配置重试异常类型和退避策略
3. **执行请求** → 尝试执行请求
4. **捕获异常** → 捕获可重试的异常
5. **判断是否重试** → 根据异常类型和重试次数决定是否重试
6. **等待退避** → 按照指数退避策略等待
7. **重新尝试** → 重新执行请求，直到成功或达到最大重试次数
