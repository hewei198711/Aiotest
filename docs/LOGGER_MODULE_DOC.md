...
- [概述](#概述)
- [核心功能](#核心功能)
- [核心类：ExtraFieldFormatter](#核心类-extrafieldformatter)
- [核心类：Logger](#核心类-logger)
- [全局实例](#全局实例)
- [调用逻辑流程](#调用逻辑流程)
- [流程图](#流程图)
- [配置参数](#配置参数)
...
2. **自动初始化** → 模块加载时自动创建 `logger` 实例
3. **创建目录** → 如果日志目录不存在，自动创建
4. **配置处理器** → 设置文件处理器和标准输出处理器
5. **设置级别** → 根据配置设置日志级别

### 日志记录流程

1. **调用日志方法** → 如 `logger.info("Message")`
2. **检查级别** → 检查消息级别是否高于或等于当前设置的级别
3. **格式化消息** → 使用 `ExtraFieldFormatter` 格式化消息
4. **输出日志** → 同时输出到文件和标准输出

### 动态日志格式流程

1. **准备额外字段** → 构造包含额外字段的字典
2. **传递额外字段** → 通过 `extra` 参数传递给日志方法
3. **检查字段** → `ExtraFieldFormatter` 检查记录是否包含特定字段
4. **选择格式** → 根据字段选择对应的格式
5. **格式化输出** → 使用选定的格式格式化并输出日志
...
| 配置项 | 类型 | 默认值 | 说明 | 适用场景 |
|-------|------|-------|------|---------|
| `name` | `str` | `'aiotest_log'` | 日志名称，用于命名日志文件 | 自定义日志文件名称 |
| `log_dir` | `str` | `'logs'` | 日志目录路径 | 自定义日志存储位置 |
| `log_level` | `str` | `'INFO'` | 日志等级，支持 DEBUG, INFO, WARNING, ERROR, CRITICAL | 控制日志详细程度 |
| `default_fmt` | `str` | `'%(asctime)s | %(levelname)8s | %(filename)s:%(lineno)d | %(message)s'` | 默认日志格式 | 标准日志输出 |
...
2. **可扩展性**：支持动态日志格式和额外字段
3. **可靠性**：防止重复添加处理器，确保日志系统的稳定性
4. **易用性**：提供简单的接口和全局实例，便于使用
5. **性能**：合理的日志级别管理和格式化逻辑，减少性能开销
