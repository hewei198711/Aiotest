...
- [数据类型和枚举](#%E6%95%B0%E6%8D%AE%E7%B1%BB%E5%9E%8B%E5%92%8C%E6%9E%9A%E4%B8%BE)
- [辅助类](#%E8%BE%85%E5%8A%A9%E7%B1%BB)
- [元类和装饰器](#%E5%85%83%E7%B1%BB%E5%92%8C%E8%A3%85%E9%A5%B0%E5%99%A8)
- [核心类：User](#%E6%A0%B8%E5%BF%83%E7%B1%BBuser)
- [扩展类：HttpUser](#%E6%89%A9%E5%B1%95%E7%B1%BBhttpuser)
...
1. **验证配置** → 验证顺序执行模式下的权重约束
1. **启动用户** → 调用 `start_tasks()` 方法
1. **初始化资源** → 调用 `on_start()` 方法
1. **执行等待** → 执行初始等待时间
1. **选择执行模式** → 根据 `execution_mode` 选择执行方式
...
1. **并发度调整**：根据系统资源和目标负载调整 `max_concurrent_tasks`，避免过度并发导致系统压力过大
