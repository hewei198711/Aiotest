...
2. **调用 initialize** → 初始化指标收集器和 Prometheus 服务
3. **启动指标收集器** → 注册事件处理器
4. **启动 Prometheus 服务** → 暴露 `/metrics` 端点
5. **添加监控任务** → 启动节点指标收集任务
6. **初始化完成** → 运行器准备就绪
...
2. **初始化协调器** → 创建 `DistributedCoordinator`
3. **调用 initialize** → 初始化指标收集器和 Prometheus 服务
4. **启动后台任务** → 启动请求监听、命令监听、心跳监听任务
5. **初始化完成** → 等待 Worker 节点连接

### 工作节点运行器初始化流程

1. **创建运行器** → 实例化 `WorkerRunner`
2. **生成 client_id** → 创建唯一标识符
3. **初始化协调器** → 创建 `DistributedCoordinator`
4. **调用 initialize** → 初始化指标收集器
5. **启动后台任务** → 启动命令监听、心跳发送、退出检查任务
6. **初始化完成** → 等待主节点命令

### 负载应用流程（Master）

1. **接收负载配置** → `apply_load(user_count, rate)`
2. **状态转换** → 从 READY 转换到 STARTING
3. **设置启动跟踪** → 初始化 `_startup_completion_tracker`
4. **广播启动命令** → `_broadcast_startup(user_count, rate)`
5. **分配资源** → `_distribute_resources()` 计算每个 Worker 的负载
6. **发送命令** → 向每个 Worker 发送启动命令
7. **等待启动完成** → `_wait_for_workers_startup_completion()`
8. **触发启动完成事件** → `startup_completed.fire()`
9. **状态转换** → 从 STARTING 转换到 RUNNING

### 负载应用流程（Worker）

1. **接收启动命令** → `_handle_command()` 处理 startup 命令
2. **调用 apply_load** → 继承自 `BaseRunner` 的方法
3. **启动用户** → `user_manager.manage_users()` 启动指定数量的用户
4. **发送启动完成确认** → `_send_startup_completed()`
5. **开始执行测试** → 用户开始执行任务

### 心跳流程

1. **Worker 发送心跳** → `_send_heartbeat()` 定期执行
2. **构建心跳数据** → 包含 CPU、用户数、状态等信息
3. **发布到 Redis** → `coordinator.publish("heartbeat", ...)`
4. **Master 接收心跳** → `_update_worker_status()` 处理
5. **更新 Worker 状态** → 更新 `WorkerNode` 对象
6. **记录指标** → 将心跳数据记录到 Prometheus
...
2. **状态转换** → 转换到 STOPPING
3. **停止指标收集器** → `metrics_collector.stop()`
4. **停止 Prometheus 服务** → `prometheus_runner.cleanup()`
5. **停止所有用户** → `user_manager.manage_users(action="stop")`
6. **取消后台任务** → `task_manager.cancel_all_tasks()`
7. **触发停止事件** → `test_stop.fire()`
8. **状态转换** → 转换到 READY
...
3. **停止指标收集器** → `metrics_collector.stop()`
4. **停止 Prometheus 服务** → `prometheus_runner.cleanup()`
5. **广播停止命令** → `coordinator.publish("command", ..., command="stop")`
6. **等待 Worker 确认** → 通过 `_handle_command` 接收 stop 确认
7. **触发停止事件** → `test_stop.fire()`
8. **状态转换** → 转换到 READY

#### 工作节点停止
1. **接收停止命令** → `_handle_command()` 处理 stop 命令
2. **调用 stop** → `WorkerRunner.stop()`
3. **状态转换** → 转换到 STOPPING
4. **停止指标收集器** → `metrics_collector.stop()`
5. **停止所有用户** → `user_manager.stop_all_users()`
6. **发送停止确认** → `_send_stop()`
7. **触发停止事件** → `test_stop.fire()`
8. **状态转换** → 转换到 READY
