...
from aiohttp.client_ws import ClientWebSocketResponse as ClientWebSocketResponse
        ...
        try:
            with timer:
                while True:
                    ...
                    request_meta = {}
                    request_meta["request_name"] = name if name else url.path
                    request_meta["request_method"] = method
            ...
            resp.request_meta = request_meta
            return resp
...
class _BaseRequestContextManager(Coroutine[Any, Any, _RetType], Generic[_RetType]):
    ...
    __slots__ = ("_coro", "_resp", "_manual_result")
    ...
    async def __aenter__(self) -> _RetType:
        ...
        self._resp.success = self.success
        self._resp.failure = self.failure
        # requests meta data
        if self._resp.headers.get("Transfer-Encoding") != "chunked":
            self._resp.request_meta["response_time"] = int((default_timer() - self._resp.request_meta["start_time"]) * 1000)
            self._resp.request_meta["response_length"] = float(self._resp.headers.get("Content-Length"))
            self._resp.request_meta.pop("start_time")
        return self._resp
    ...
    def success(self):
        ...
        Example::
            ...
            async with self.sessen.get("/does/not/exist") as resp:
                if resp.status == 404:
                    resp.success()
        """
    ...
    def failure(self, exc):
        ...
        Example::
            ...
            async with self.sessen.get("/") as resp:
                if resp.status == 200:
                    response.failure("is 200")
        """
...
class _RequestContextManager(_BaseRequestContextManager[ClientResponse]):
    ...
    async def __aexit__(
        ...
        # requests meta data
        if self._resp.request_meta.get("start_time"):
            self._resp.request_meta["response_time"] = int((default_timer() - self._resp.request_meta["start_time"]) * 1000)
            self._resp.request_meta["response_length"] = len(self._resp._body or "")
            self._resp.request_meta.pop("start_time")
        self._resp.request_meta["error"] = None
